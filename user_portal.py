...
# ══════════════════════════════════════
# CONFIG — reads from st.secrets only, no hardcoded fallback
# ══════════════════════════════════════
...
.stTextInput > div > div > input:focus {
    border-color: #a2ff00 !important; box-shadow: 0 0 0 2px rgba(162,255,0,.1) !important;
}
.stTextInput label {
    font-family: 'Space Mono', monospace !important; font-size: 10px !important;
    color: #4a6070 !important; text-transform: uppercase !important; letter-spacing: .08em !important;
}
div.stButton > button {
...
.nx-card {
    ...
    border: 1px solid #182230; border-radius: 14px; padding: 24px; margin-bottom: 14px;
}
.nx-card-title {
    font-family: 'Space Mono', monospace; font-size: 9px; color: #4a6070;
    text-transform: uppercase; letter-spacing: .12em; margin-bottom: 16px;
}
.nx-notice {
    background: rgba(255,179,0,.05); border: 1px solid rgba(255,179,0,.2);
    border-left: 3px solid #ffb300; border-radius: 0 8px 8px 0;
    padding: 10px 14px; font-family: 'Space Mono', monospace; font-size: 9px;
    color: #ffb300; line-height: 1.7; margin-bottom: 18px;
}
.nx-rank-wrap { display: flex; flex-direction: column; align-items: center; padding: 32px 0 24px; }
...
.nx-rank-num { font-family: 'Space Mono', monospace; font-size: 28px; font-weight: 700; color: #a2ff00; line-height: 1; }
.nx-rank-label { font-family: 'Space Mono', monospace; font-size: 8px; color: #4a6070; text-transform: uppercase; letter-spacing: .1em; margin-top: 4px; }
.nx-rank-title { font-size: 18px; font-weight: 800; color: #e8edf2; margin-bottom: 4px; }
.nx-rank-sub { font-family: 'Space Mono', monospace; font-size: 10px; color: #4a6070; text-align: center; line-height: 1.6; }
.nx-ref-box { background: #060b0f; border: 1px solid rgba(162,255,0,.25); border-radius: 12px; padding: 20px; text-align: center; margin-bottom: 14px; }
.nx-ref-code { font-family: 'Space Mono', monospace; font-size: 28px; font-weight: 700; color: #a2ff00; letter-spacing: .25em; margin: 8px 0 4px; }
.nx-ref-label { font-family: 'Space Mono', monospace; font-size: 8px; color: #4a6070; text-transform: uppercase; letter-spacing: .1em; }
.nx-ref-count { font-family: 'Space Mono', monospace; font-size: 12px; color: #00e5ff; margin-top: 10px; }
.nx-stat-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 14px; }
.nx-stat-item { background: #060b0f; border: 1px solid #182230; border-radius: 10px; padding: 16px; text-align: center; }
.nx-stat-val { font-family: 'Space Mono', monospace; font-size: 20px; font-weight: 700; color: #e8edf2; line-height: 1.1; }
.nx-stat-val.green { color: #a2ff00; }
.nx-stat-val.cyan  { color: #00e5ff; }
.nx-stat-val.gold  { color: #ffb300; }
.nx-stat-label { font-family: 'Space Mono', monospace; font-size: 8px; color: #4a6070; text-transform: uppercase; letter-spacing: .08em; margin-top: 5px; }
.nx-share-row { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin-top: 12px; }
.nx-share-btn { display: block; text-align: center; padding: 10px 8px; background: #0d1720; border: 1px solid #182230; border-radius: 10px; color: #4a6070 !important; font-size: 11px; font-weight: 700; text-decoration: none; transition: all .2s; }
.nx-share-btn:hover { border-color: #a2ff00; color: #a2ff00 !important; }
.nx-timeline { padding: 4px 0; }
.nx-tl-item { display: flex; gap: 14px; padding-bottom: 20px; position: relative; }
.nx-tl-item::before { content: ''; position: absolute; left: 9px; top: 22px; bottom: 0; width: 1px; background: #182230; }
.nx-tl-item:last-child::before { display: none; }
.nx-tl-dot { width: 20px; height: 20px; border-radius: 50%; background: #182230; border: 2px solid #182230; flex-shrink: 0; margin-top: 2px; }
.nx-tl-dot.done  { background: #a2ff00; border-color: #a2ff00; }
.nx-tl-dot.now   { background: #00e5ff; border-color: #00e5ff; box-shadow: 0 0 10px rgba(0,229,255,.5); }
.nx-tl-title { font-size: 13px; font-weight: 700; color: #e8edf2; line-height: 1.3; }
.nx-tl-title.muted { color: #2a3a4a; }
.nx-tl-sub { font-family: 'Space Mono', monospace; font-size: 9px; color: #4a6070; margin-top: 3px; line-height: 1.6; }
.nx-login-hero { text-align: center; padding: 40px 0 32px; }
.nx-login-dot { width: 14px; height: 14px; background: #a2ff00; border-radius: 50%; box-shadow: 0 0 18px #a2ff00; margin: 0 auto 20px; }
.nx-login-title { font-size: 28px; font-weight: 800; color: #e8edf2; letter-spacing: -.02em; margin-bottom: 6px; }
.nx-login-sub { font-family: 'Space Mono', monospace; font-size: 10px; color: #4a6070; line-height: 1.7; max-width: 340px; margin: 0 auto 32px; }
.nx-stage { display: inline-block; background: rgba(162,255,0,.08); border: 1px solid rgba(162,255,0,.25); color: #a2ff00; font-family: 'Space Mono', monospace; font-size: 9px; font-weight: 700; padding: 4px 10px; border-radius: 6px; letter-spacing: .1em; margin-bottom: 20px; }
...
PORTAL_TEXT = {
    "EN": {
        "title": "NODE PORTAL", "stage": "● BETA · Q3 2026",
        "not_on_waitlist": "Your email is not on the waitlist yet. Please register at the main site first.",
        "sign_out": "Sign Out", "node_reserved": "Your node is reserved.",
        "top_pct": "Top", "of": "of", "waitlist_members": "waitlist members",
        "joined": "Joined", "referrals_made": "Referrals Made",
        "queue_position": "Queue Position", "spl_wallet": "SPL Wallet", "language": "Language",
        "your_ref": "Your Referral Code", "joined_with_code": "person joined with your code",
        "joined_with_code_pl": "persons joined with your code",
        "tap_copy": "Tap & hold to copy your referral code",
        "mining_status": "Mining Status", "token_earnings": "Token Earnings",
        "participant_node": "Participant Node", "start_session": "⚡ START COMPUTE SESSION",
        "stop": "■ STOP", "online_refresh": "ONLINE · refreshes every 30s",
        "press_start": "// Press START COMPUTE SESSION to activate node",
        "task_history": "Task History", "completed": "completed",
        ...
        "wasm_title": "WASM COMPUTE DEMO", "wasm_badge": "RUNS IN YOUR BROWSER",
        ...
        "nodes_reserved": "nodes reserved", "email_label": "Email Address",
        "email_ph": "you@example.com", "send_code": "Send Login Code",
        "invalid_email": "Please enter a valid email address.",
        "not_on_wl": "This email is not on the waitlist. Please register at the main site first.",
        "your_code": "Your login code", "signing_in": "Signing in as",
        "beta_warning": "BETA MODE — Code shown on screen. Valid 10 minutes.",
        "enter_code": "Enter the 6-digit code above", "code_ph": "e.g. 123456",
        "verify": "Verify & Sign In", "wrong_code": "Incorrect code. Please try again.",
        "diff_email": "← Use a different email", "lang_btn": "中文",
    ...
    "ZH": {
        "title": "节点控制台", "stage": "● Beta · 2026年Q3",
        "not_on_waitlist": "您的邮箱尚未在候补名单中。请先在主网站注册。",
        "sign_out": "退出登录", "node_reserved": "您的节点已预留。",
        "top_pct": "前", "of": "/", "waitlist_members": "位候补名单成员",
        "joined": "加入于", "referrals_made": "已推荐人数",
        "queue_position": "队列排名", "spl_wallet": "SPL 钱包", "language": "语言",
        "your_ref": "您的推荐码", "joined_with_code": "人使用了您的推荐码",
        "joined_with_code_pl": "人使用了您的推荐码",
        "tap_copy": "长按复制推荐码",
        "mining_status": "挖矿状态", "token_earnings": "代币收益",
        "participant_node": "参与节点", "start_session": "⚡ 启动算力会话",
        "stop": "■ 停止", "online_refresh": "在线 · 每30秒刷新",
        "press_start": "// 按启动算力会话激活节点",
        "task_history": "任务历史", "completed": "已完成",
        "register_device": "将此设备注册为节点",
        "register_desc": "为此设备生成唯一节点 Token。使用此 Token 运行节点客户端，开始发送心跳并执行任务。",
        "register_btn": "⚡ 注册此设备",
        "register_fail": "注册失败。您可能已经注册了节点。",
        "wasm_title": "WASM 算力演示", "wasm_badge": "在您的浏览器中运行",
        ...
        "nodes_reserved": "个节点已预留", "email_label": "电子邮件",
        "email_ph": "your@email.com", "send_code": "发送登录验证码",
        "invalid_email": "请输入有效的电子邮件地址。",
        "not_on_wl": "此邮箱不在候补名单中。请先在主网站注册。",
        "your_code": "您的登录验证码", "signing_in": "正在登录",
        "beta_warning": "Beta 模式 — 验证码显示在屏幕上。有效期10分钟。",
        "enter_code": "输入上方的6位验证码", "code_ph": "如 123456",
        "verify": "验证并登录", "wrong_code": "验证码错误，请重试。",
        "diff_email": "← 使用其他邮箱", "lang_btn": "English",
...
# ══════════════════════════════════════
for k, v in {
    "user_email": None, "user_data": None,
    "magic_sent": False, "magic_email": "", "otp_store": {},
}.items():
...
def register_node(email, node_token):
    try:
        supabase.table("nodes").insert({
            "email": email.lower(), "node_token": node_token, "status": "pending",
        }).execute()
        return True
    except: return False
...
def get_latest_heartbeat(token):
    try:
        res = (supabase.table("heartbeats")
               ...
               .eq("node_token", token).order("reported_at", desc=True).limit(1).execute())
        return res.data[0] if res.data else None
    except: return None
...
def get_node_tasks(token, limit=10):
    try:
        res = (supabase.table("tasks")
               ...
               .eq("assigned_to", token).order("created_at", desc=True).limit(limit).execute())
        return res.data or []
    except: return []

def get_node_task_count(token):
    try:
        res = (supabase.table("tasks").select("id", count="exact")
               .eq("assigned_to", token).eq("status", "completed").execute())
        return res.count or 0
    except: return 0
...
if st.session_state.user_email:
    ...
    if not data:
        st.markdown(f'<div class="nx-notice">{T["not_on_waitlist"]}</div>', unsafe_allow_html=True)
    ...
    <div class="nx-stat-grid">
        <div class="nx-stat-item"><div class="nx-stat-val green">{referrals}</div><div class="nx-stat-label">{T['referrals_made']}</div></div>
        <div class="nx-stat-item"><div class="nx-stat-val cyan">{rank} / {total}</div><div class="nx-stat-label">{T['queue_position']}</div></div>
        <div class="nx-stat-item"><div class="nx-stat-val">{wallet_short}</div><div class="nx-stat-label">{T['spl_wallet']}</div></div>
        <div class="nx-stat-item"><div class="nx-stat-val gold">{lang}</div><div class="nx-stat-label">{T['language']}</div></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if is_zh:
        ...
        wa_msg          = f"加入 NexaEdge 候补名单 — 将闲置手机算力变成分布式 AI 网络。使用我的推荐码 {ref_code} . {x_site_url}"
        ...
        share_text      = f"{share_text_only}\n{x_site_url}"
        wa_msg          = f"Join the NexaEdge waitlist — distributed edge AI on smartphones. Use my code {ref_code} . {x_site_url}"
        tg_url          = "https://telegram.me/share/url?url=" + urllib.parse.quote(x_site_url) + "&text=" + urllib.parse.quote(share_text)
        wa_url          = "https://wa.me/?text=" + urllib.parse.quote(wa_msg)
        x_url           = "https://x.com/intent/tweet?text=" + urllib.parse.quote(share_text_only) + "&url=" + urllib.parse.quote(x_site_url)
    ...
    <div class="nx-ref-box">
        ...
        <div class="nx-ref-count">{referrals} {T['joined_with_code_pl'] if referrals != 1 else T['joined_with_code']}</div>
    </div>
    ...
    st.text_input(copy_hint, value=share_text, key="ref_code_display", label_visibility="visible")
    ...
    if node_rec:
        ...
        mining_color   = '#a2ff00' if is_online else '#ffb300'
        mining_text    = '● ACTIVE' if is_online else '○ ' + status.upper()
        ...
        * {{ margin:0;padding:0;box-sizing:border-box;font-family:'Space Mono',monospace; }}
        ...
        <div class="dash">
          <div class="dash-header">
            <div><div class="dash-title">Dashboard</div><div style="font-size:9px;color:#4a6070;margin-top:2px;">Network Hash Rate (MH/s)</div></div>
            <div class="temp-badge" style="background:{temp_bg};color:{temp_color};">{temp:.1f}C &nbsp; {temp_label}</div>
          </div>
          ...
          <div class="stats-row">
            <div class="stat-cell"><div class="stat-num" style="color:#a2ff00;">{cpu:.1f}%</div><div class="stat-lbl">CPU</div></div>
            <div class="stat-cell"><div class="stat-num" style="color:{temp_color};">{temp:.1f}C</div><div class="stat-lbl">Temp</div></div>
            <div class="stat-cell"><div class="stat-num">{batt}%</div><div class="stat-lbl">Battery</div></div>
          </div>
          <div class="node-section">
            <div class="node-lbl">{T['participant_node']}</div>
            <div class="node-id">NODE_ID: @nexaedge / {token[-12:]}</div>
            <div class="status-row">
              <div class="status-block">
                <div class="status-lbl">{T['mining_status']}</div>
                <div class="status-val" style="color:{mining_color};">{mining_text}</div>
              </div>
        ...
        if node_active:
            ...
            try:
                ...
                supabase.table("heartbeats").insert({
                    "node_token": token, "cpu_usage": cpu_sim, "temperature": temp_sim,
                    "battery_level": 100, "tasks_completed": st.session_state.get("node_tasks", 0),
                    "reported_at": now_iso,
                }).execute()
                supabase.table("nodes").update({"status": "online", "last_seen": now_iso}).eq("node_token", token).execute()
                hb_msg   = f"♥ CPU {cpu_sim}%  Temp {temp_sim}°C  Batt 100%"
                hb_color = "#a2ff00"
            ...
            task_msg = None
            try:
                ttype  = random.choice(["slm_inference", "rlhf_validation", "zk_proof"])
                result = f"[Portal] {ttype} OK | latency={round(random.uniform(2,5),1)}ms | node={token[-8:]}"
                supabase.table("tasks").insert({
                    "task_type": ttype, "status": "completed", "assigned_to": token,
                    "payload": f"auto_{random.randint(1000,9999)}", "result": result,
            ...
            log_html = "".join(f'<div style="color:{c};line-height:1.9;">{l}</div>' for l, c in log)
            st.markdown(f"""
            <div style="background:#040709;border:1px solid #182230;border-radius:10px;padding:14px;font-family:'Space Mono',monospace;font-size:10px;margin-top:4px;">
            ...
            st.markdown(f'<div style="background:#040709;border:1px solid #182230;border-radius:10px;padding:14px;font-family:\'Space Mono\',monospace;font-size:10px;color:#2a3a4a;margin-top:4px;">{T["press_start"]}</div>', unsafe_allow_html=True)
        ...
        if tasks:
            ...
            for t in tasks[:8]:
                ...
                rows += (
                    '<div style="display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #182230;">'
            ...
            st.components.v1.html(
                ...
                '<div style="background:linear-gradient(160deg,#0d1720,#090e14);border:1px solid #182230;border-radius:14px;padding:14px 16px;">'
                f'<div style="font-family:monospace;font-size:10px;color:#4a6070;text-transform:uppercase;letter-spacing:.1em;margin-bottom:10px;">&#9658; {T["task_history"]} <span style="color:#a2ff00;">{task_count} {T["completed"]}</span></div>'
                + rows + '</div>',
                height=min(55 + len(tasks[:8]) * 36, 380)
            )
    else:
        st.markdown(f"""
        <div class="nx-card">
            <div class="nx-card-title">▸ {T['register_device']}</div>
            <div style="font-size:12px;color:#4a6070;line-height:1.7;margin-bottom:16px;">{T['register_desc']}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div class="nx-card" style="border-color:rgba(0,229,255,.2);">
        <div class="nx-card-title"><span style="color:#00e5ff;">▸</span> {T['wasm_title']}
            <span style="background:rgba(0,229,255,.1);border:1px solid rgba(0,229,255,.2);color:#00e5ff;font-family:'Space Mono',monospace;font-size:8px;padding:2px 7px;border-radius:4px;margin-left:6px;">{T['wasm_badge']}</span>
        </div>
        <div style="font-size:11px;color:#4a6070;line-height:1.7;margin-bottom:14px;">{T['wasm_desc']}</div>
    </div>
    """, unsafe_allow_html=True)

    _wasm_node = node_rec.get("node_token", "—")[-12:] if node_rec else "—"
    st.components.v1.html(f"""
    <div style="background:#040709;border:1px solid rgba(0,229,255,.2);border-radius:10px;padding:16px;font-family:'Space Mono',monospace;font-size:10px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
            <div id="wasm-dot" style="width:8px;height:8px;border-radius:50%;background:#4a6070;transition:background .3s;"></div>
            <div id="wasm-status" style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#4a6070;">READY</div>
        </div>
        <div id="wasm-log" style="color:#2a3a4a;line-height:1.9;min-height:40px;font-size:10px;margin-bottom:14px;">// Press RUN to execute compute kernel in this browser</div>
        <div style="display:flex;gap:8px;margin-bottom:12px;">
            <button onclick="runCompute(32)" style="background:linear-gradient(135deg,#00e5ff,#0099bb);color:#060b0f;border:none;border-radius:6px;padding:8px 16px;font-family:monospace;font-size:10px;font-weight:700;letter-spacing:.06em;cursor:pointer;">▶ RUN (32×32)</button>
            <button onclick="runCompute(128)" style="background:transparent;color:#00e5ff;border:1px solid rgba(0,229,255,.3);border-radius:6px;padding:8px 16px;font-family:monospace;font-size:10px;cursor:pointer;">⚡ STRESS (128×128)</button>
        </div>
        <div id="wasm-result" style="display:none;background:#060b0f;border:1px solid #182230;border-radius:8px;padding:12px;margin-top:8px;">
            <div style="font-size:9px;color:#4a6070;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;">EXECUTION RESULT</div>
    ...
    function matmul(N) {{
        ...
        let cs = 0; for (let i = 0; i < N*N; i++) cs += C[i]; return cs;
    }}
    function runCompute(N) {{
        ...
        setTimeout(() => {{
            ...
            const lat = (t1-t0).toFixed(2);
            const gf  = (2*N*N*N/((t1-t0)/1000)/1e9).toFixed(2);
            setStatus("COMPUTE DONE", "#a2ff00");
            log(N+"×"+N+" done · "+lat+"ms · "+gf+" GFLOPS", "#a2ff00");
            ...
            document.getElementById("wasm-output").innerHTML =
                ...
                "Latency: <span style='color:#a2ff00;'>" + lat + "ms</span><br>" +
                "Throughput: <span style='color:#00e5ff;'>" + gf + " GFLOPS</span><br>" +
                "Checksum: <span style='color:#4a6070;'>" + cs.toFixed(4) + "</span><br>" +
    ...
    <div class="nx-card">
        ...
        <div class="nx-timeline">
            <div class="nx-tl-item"><div class="nx-tl-dot done"></div><div><div class="nx-tl-title">{T['j1_title']}</div><div class="nx-tl-sub">{T['j1_sub']}</div></div></div>
            <div class="nx-tl-item"><div class="nx-tl-dot done"></div><div><div class="nx-tl-title">{T['j2_title']}</div><div class="nx-tl-sub">{T['j2_sub']}</div></div></div>
            <div class="nx-tl-item"><div class="nx-tl-dot now"></div><div><div class="nx-tl-title">{T['j3_title']}</div><div class="nx-tl-sub">{T['j3_sub']}</div></div></div>
            <div class="nx-tl-item"><div class="nx-tl-dot"></div><div><div class="nx-tl-title muted">{T['j4_title']}</div><div class="nx-tl-sub">{T['j4_sub']}</div></div></div>
            <div class="nx-tl-item"><div class="nx-tl-dot"></div><div><div class="nx-tl-title muted">{T['j5_title']}</div><div class="nx-tl-sub">{T['j5_sub']}</div></div></div>
        </div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown(f'<div class="nx-notice">⚠ {T["nexa_notice"]}</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════
    ...
    <div class="nx-login-hero">
        ...
        <div style="margin-bottom:10px;"><span class="nx-stage">{T['stage']}</span></div>
