...
def generate_referral_image(referral_code: str, image_path: str = None) -> bytes:
    """
    ...
    if image_path and os.path.exists(image_path):
        ...
        img = Image.new("RGBA", (520, 520), color=(11, 15, 18, 255))
...
st.markdown("""
    ...
    /* 推荐码图片容器 */
    .ref-img-card {
        background: linear-gradient(135deg, #0d1f0d 0%, #111c11 100%);
        border: 1px dashed #A2FF00;
        border-radius: 16px;
        padding: 16px;
        text-align: center;
        margin: 10px 0;
...
with tab1:
    ...
    # ── 显示白名单成功后生成的推荐码图片（持久展示，直到下次提交） ──
    if st.session_state.wl_referral_image_bytes and st.session_state.wl_referral_code:
        render_referral_image_block(
            st.session_state.wl_referral_image_bytes,
            st.session_state.wl_referral_code,
            lang,
            dl_key="wl_img_dl_persistent"
        )
    ...
    with st.form("unified_whitelist_form"):
        ...
        if st.form_submit_button(btn_wl_txt):
            if not u_email or not u_wallet:
                ...
                wl_img_bytes = generate_referral_image(wl_ref_code, target_image)
                st.session_state.wl_referral_image_bytes = wl_img_bytes
                st.session_state.wl_referral_code = wl_ref_code

                # 内嵌展示（当次表单内即时可见）
                img_b64 = base64.b64encode(wl_img_bytes).decode()
                label_title = "📸 您的专属推荐码图片已生成，长按保存！" if lang == "中文" else "📸 Your referral card is ready — tap & hold to save!"
                st.markdown(f"""
                <div class="ref-img-card">
                    <p style="color:#A2FF00; font-size:12px; font-weight:bold; margin-bottom:8px;">{label_title}</p>
                    <img src="data:image/png;base64,{img_b64}"
                         style="border-radius:12px; border:1px solid #A2FF00;
                                max-width:280px; width:100%; box-shadow:0 0 16px rgba(162,255,0,0.25);" />
                </div>
                """, unsafe_allow_html=True)
                st.download_button(
                    label="⬇️ 下载推荐码图片" if lang == "中文" else "⬇️ Download Referral Card",
                    data=wl_img_bytes,
                    file_name=f"NexaEdge_Whitelist_{wl_ref_code}.png",
                    mime="image/png",
                    key="wl_img_dl_inline"
                )
...
with tab4:
    if st.session_state.current_user:
        ...
        if st.session_state.referral_image_bytes:
            # 注册后首次：直接展示已生成的图片
            render_referral_image_block(
                st.session_state.referral_image_bytes,
                ref_code,
                lang,
                dl_key="reg_img_dl"
            )
            ...
            if st.button(gen_lbl, key="gen_img_btn"):
                with st.spinner("生成中..." if lang == "中文" else "Generating..."):
                    st.session_state.referral_image_bytes = generate_referral_image(ref_code, target_image)
                st.rerun()
        ...
        if st.button(btn_logout, key="logout_btn"):
            st.session_state.current_user = None
            st.session_state.app_running = False
            st.session_state.referral_image_bytes = None  # 退出时清除图片缓存
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
            ...
            with st.form("login_form"):
                ...
                if st.form_submit_button(btn_l_txt):
                    ...
                    if l_email in global_server["user_db"] and global_server["user_db"][l_email]["password_hash"] == p_hash:
                        ...
                        st.session_state.referral_image_bytes = None  # 登录时清空，等用户按需生成
