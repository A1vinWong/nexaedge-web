...
with tab4:
    if st.session_state.current_user:
        ...
        if st.button("安全退出当前登录账户" if lang=="中文" else "Logout Account", key="logout_btn"):
        ...
        if auth_mode in ["注册新节点账户", "Register Node Account"]:
            ...
            with st.form("login_form"):
                st.markdown(f'<div style="font-size:12px; font-weight:bold; color:#00e5ff; margin-bottom:6px;">{"🔑 登录 NexaEdge 算力账户" if lang=="中文" else "🔑 Connect Node Endpoint Terminal"}</div>', unsafe_allow_html=True)
...
if is_admin_active:
    ...
    if admin_password == "NexaAdmin2026":
        ...
        with adm_sub_tab2:
            if whitelist_lines:
                wl = """<table class="admin-table"><tr><th>序号</th><th>提交的日志 data</th></tr>"""
