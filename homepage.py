...
st.markdown("""
    ...
    [data-testid="stVerticalBlock"] > div { padding-bottom: 0.15rem !important; padding-top: 0.15rem !important; }
    ...
    .chart-wrapper { background-color: #161c23; border: 1px solid #252e38; border-radius: 14px; padding: 8px 10px 0px 10px; margin-top: 4px; margin-left: -18px; margin-right: 8px; margin-bottom: 8px; box-sizing: border-box; overflow: hidden; }
    ...
    .glow-ref-code { color: #A2FF00; text-shadow: 0 0 10px rgba(162,255,0,0.8), 0 0 20px rgba(162,255,0,0.5); font-family: 'Courier New', monospace; font-weight: 900; font-size: 20px; }
    .disclaimer-bar { background: rgba(255,179,0,0.08); border: 1px solid rgba(255,179,0,0.25); border-radius: 8px; padding: 6px 12px; font-size: 10px; color: #88929b; text-align: center; margin: 4px 0 8px 0; line-height: 1.5; }
    .sim-badge { display: inline-block; background: rgba(255,179,0,0.12); color: #ffb300; font-size: 9px; font-weight: bold; padding: 1px 6px; border-radius: 3px; border: 1px solid rgba(255,179,0,0.3); vertical-align: middle; margin-left: 4px; letter-spacing: 0.05em; }
...
if st.session_state.app_running and st.session_state.last_tick_time > 0:
    ...
    if elapsed_gap >= 1:
        ...
        st.session_state.app_earned += execute_secure_wasm_task(1) * elapsed_gap
...
if lang == "中文":
    st.markdown('<div style="background:#11171d;border:1px solid #1e272e;border-radius:12px;padding:12px 14px;margin:6px 0 8px 0;"><div style="color:#A2FF00;font-size:13px;font-weight:900;letter-spacing:1px;margin-bottom:6px;">⚡ 项目简介</div><div style="color:#bdc3c7;font-size:12px;line-height:1.6;">NexaEdge 让用户将闲置手机算力变现。通过构建加密去中心化沙盒网络，您的设备在睡眠时无缝处理本地数据验证任务，解锁机构级收益回报。</div></div>', unsafe_allow_html=True)
else:
    st.markdown('<div style="background:#11171d;border:1px solid #1e272e;border-radius:12px;padding:12px 14px;margin:6px 0 8px 0;"><div style="color:#A2FF00;font-size:13px;font-weight:900;letter-spacing:1px;margin-bottom:6px;">⚡ PROJECT BRIEFING</div><div style="color:#bdc3c7;font-size:12px;line-height:1.6;">NexaEdge empowers users to monetize unutilized smartphone capabilities. By creating an encrypted decentralized sandbox network, your device seamlessly routes localized data verification processes to unlock institutional level rewards while you sleep.</div></div>', unsafe_allow_html=True)
...
with tab1:
    ...
    if lang == "中文":
        ...
        st.markdown("""
        <div class="feature-box"><h4 style="color:white;margin:0;font-size:13px;">📱 充电即赚 · 睡后收入</h4><p style="color:#bdc3c7;font-size:11px;margin:2px 0 0 0;">只需在夜间充电并连接 Wi-Fi，NexaEdge 的轻量级 WASM 沙盒便会在后台静默运行清洗 AI 语料。</p></div>
        <div class="feature-box"><h4 style="color:white;margin:0;font-size:13px;">🔥 39°C 智能温控屏障</h4><p style="color:#bdc3c7;font-size:11px;margin:2px 0 0 0;">坚守绝不伤机底线。一旦手机运行温度触及 39°C 临界点，系统自动下发降载指令，打消损耗焦虑。</p></div>
        <div class="feature-box"><h4 style="color:white;margin:0;font-size:13px;">🔗 自研轻量级拜占庭容错共识</h4><p style="color:#bdc3c7;font-size:11px;margin:2px 0 0 0;">专为边缘不可信设备设计，采用 2:1 去中心化多数投票冗余验证机制，确保每一份算力贡献真实可信、不可篡改。</p></div>
        """, unsafe_allow_html=True)
    else:
        ...
        st.session_state.target_time_index = current_options.index(selected_time_tab1)
        st.info(f"📊 Simulated monthly reference: {HOURS_MAP[st.session_state.target_time_index]*0.35*30:.2f} USDT — Testnet simulation only, not an earnings guarantee.")
        st.markdown("""
        <div class="feature-box"><h4 style="color:white;margin:0;font-size:13px;">📱 Passive Income via Charging</h4><p style="color:#bdc3c7;font-size:11px;margin:2px 0 0 0;">Just plug in and connect Wi-Fi at night, NexaEdge's lightweight WASM Sandbox cleans AI datasets silently.</p></div>
        <div class="feature-box"><h4 style="color:white;margin:0;font-size:13px;">🔥 39°C Thermal Guard Barrier</h4><p style="color:#bdc3c7;font-size:11px;margin:2px 0 0 0;">Total hardware protection. System auto-throttles load instantly if battery hits 39°C.</p></div>
        <div class="feature-box"><h4 style="color:white;margin:0;font-size:13px;">🔗 Proprietary Lightweight BFT Consensus</h4><p style="color:#bdc3c7;font-size:11px;margin:2px 0 0 0;">Built for untrusted edge devices — 2:1 Decentralized Majority Voting Redundant Verification ensures every compute contribution is authentic and tamper-proof.</p></div>
        """, unsafe_allow_html=True)
    ...
    with st.form("unified_whitelist_form"):
        if lang == "中文":
            ...
            u_wallet_label="绑定的 Solana 钱包接收地址:"; u_wallet_place="输入您的 Solana SPL 钱包公钥"
            u_ref_label="推荐码 (选填):"; u_ref_place="如有推荐码，请输入以获得创世加速"
            btn_wl_txt="锁定创世早鸟席位 ⚡"; msg_empty="❌ 请完整填写邮箱和钱包地址！"
            msg_success="🎉 早鸟席位登记成功！主网上线前我们将与您联系。"; contact_btn_label="📧 联系我们"
        else:
            ...
            u_wallet_label="Bound Solana Wallet Address:"; u_wallet_place="Enter your Solana SPL public key address"
            u_ref_label="Referral Code (Optional):"; u_ref_place="Enter code to claim genesis boosting if available"
            btn_wl_txt="Secure Early Access Seat ⚡"; msg_empty="❌ Email and Wallet fields cannot be empty!"
            msg_success="🎉 Early access registered! We'll reach out before mainnet launch."; contact_btn_label="📧 Contact US"
...
with tab2:
    if st.session_state.app_running:
        ...
        current_hash = 0.0; current_temp = 30.5
    ...
    st.markdown(f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;margin-left:14px;padding-right:8px;"><div class="chart-title-lbl" style="margin-left:0px;padding-left:0px;">{chart_lbl}</div><div style="{sandbox_style}">{sandbox_status}</div></div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="app-card"><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;"><div style="background:#11171d;padding:6px;border-radius:8px;"><div style="font-size:9px;color:#88929b;font-weight:bold;">{lbl_p1}</div><div class="app-value neon-blue-text" style="font-size:13px;font-family:monospace;">{current_power:.2f} W</div></div><div style="background:#11171d;padding:6px;border-radius:8px;"><div style="font-size:9px;color:#88929b;font-weight:bold;">{lbl_p2}</div><div class="app-value" style="font-size:13px;font-family:monospace;color:#ffffff;">{st.session_state.total_energy_wh:.4f} Wh</div></div></div></div>', unsafe_allow_html=True)
    ...
    status_badge = '<span style="background-color:#1e272e;color:#A2FF00;font-size:10px;font-weight:bold;padding:2px 6px;border-radius:4px;margin-left:8px;vertical-align:middle;">ACTIVE</span>' if st.session_state.app_running else '<span style="background-color:#1e272e;color:#88929b;font-size:10px;font-weight:bold;padding:2px 6px;border-radius:4px;margin-left:8px;vertical-align:middle;">STANDBY</span>'

    st.markdown(f'<div class="app-card"><div style="display:flex;justify-content:space-between;align-items:flex-end;gap:8px;"><div style="flex:1;min-width:0;"><div style="font-size:9px;color:#88929b;font-weight:bold;">{lbl_d1}</div><div class="app-value" style="font-size:14px;display:inline-block;white-space:nowrap;">{time_str}{status_badge}</div></div><div style="text-align:right;flex:1;min-width:0;"><div style="font-size:9px;color:#88929b;font-weight:bold;">{lbl_d2}</div><div class="app-value neon-green-text" style="font-size:14px;white-space:nowrap;">{st.session_state.app_earned:,.2f}</div></div></div></div>', unsafe_allow_html=True)
...
with tab4:
    if st.session_state.current_user:
        ...
        if st.button("安全退出当前账户" if lang=="中文" else "Logout Account", key="logout_btn"):
        ...
        if auth_mode in ["注册新节点账户","Register Node Account"]:
            with st.form("reg_form"):
                ...
                r_email = st.text_input("邮箱地址:" if lang=="中文" else "Email Address:", placeholder="example@nexa.com").strip()
                r_pwd = st.text_input("设置密码:" if lang=="中文" else "Choose Password:", type="password", placeholder="Password")
                r_ref = st.text_input("推荐码 (选填):" if lang=="中文" else "Referral Code (Optional):", placeholder="e.g., NX-XXX-XXX").strip()
            ...
            with st.form("login_form"):
                st.markdown(f'<div style="font-size:12px;font-weight:bold;color:#00e5ff;margin-bottom:6px;">{"🔑 登录 NexaEdge 账户" if lang=="中文" else "🔑 Login to NexaEdge Account"}</div>', unsafe_allow_html=True)
                ...
                if st.form_submit_button("登录账户 ⚡" if lang=="中文" else "Login ⚡"):
...
if is_admin_active:
    ...
    if admin_password == "NexaAdmin2026":
        ...
        with c_a2: st.markdown(f'<div class="mini-stat-card" style="border:1px solid #ffb300;"><div class="mini-stat-title">🎁 Whitelist Claims</div><div class="mini-stat-value" style="color:#ffb300;">{len(whitelist_lines)}</div></div>', unsafe_allow_html=True)
        ...
        with adm_sub_tab2:
            if whitelist_lines:
                wl = """<table class="admin-table"><tr><th>#</th><th>Entry</th></tr>"""
                for idx, line in enumerate(whitelist_lines, 1):
                    wl += f"<tr><td>{idx}</td><td style='font-family:monospace;color:#bdc3c7;'>{line}</td></tr>"
                st.markdown(wl + "</table>", unsafe_allow_html=True)
            else:
...
# ── 底栏 ──
lbl_active_nodes = "● 全网活跃节点" if lang=="中文" else "● ACTIVE NODES"
lbl_real_viewers = "👀 实时在线" if lang=="中文" else "👀 LIVE VIEWERS"
